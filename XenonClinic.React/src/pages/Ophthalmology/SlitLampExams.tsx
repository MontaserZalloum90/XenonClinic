...
const slitLampApi = {
  ...
  create: (data: CreateSlitLampExamRequest) =>
...
export const SlitLampExams = () => {
  ...
  const [selectedExam, setSelectedExam] = useState<SlitLampExam | undefined>(
    undefined,
  );
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: number;
      data: Partial<CreateSlitLampExamRequest>;
  ...
  const handleDelete = () => {
    if (
      window.confirm("Are you sure you want to delete this slit lamp exam?")
    ) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Anterior segment examination records
          </p>
        </div>
                ...
                {filteredExams.map((exam) => (
                  <tr key={exam.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {exam.findings.length > 50
                        ? `${exam.findings.substring(0, 50)}...`
                        : exam.findings}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {exam.cornea || "-"}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setFormData({ ...formData, cornea: e.target.value })
            }
            className="input w-full"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setFormData({ ...formData, anteriorChamber: e.target.value })
            }
            className="input w-full"
