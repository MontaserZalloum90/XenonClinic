...
export const GlassesPrescriptions = () => {
  ...
  const [selectedPrescription, setSelectedPrescription] = useState<GlassesPrescription | undefined>(
    undefined
  );
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateGlassesPrescriptionRequest) => prescriptionApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<CreateGlassesPrescriptionRequest> }) =>
  ...
  const filteredPrescriptions = prescriptions?.filter(
    (prescription) =>
      prescription.patient?.fullNameEn.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const formatPrescription = (sphere: number, cylinder: number, axis: number, add?: number) => {
  ...
  const activePrescriptions =
    prescriptions?.filter(
      (p) => p.status === undefined || p.status === PrescriptionStatusEnum.Active
    ).length || 0;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Glasses Prescriptions</h1>
          <p className="text-gray-600 mt-1">Manage eyeglass prescriptions and dispensing</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="card">
          ...
          <p className="text-2xl font-bold text-green-600 mt-1">{activePrescriptions}</p>
        </div>
        <div className="card">
                ...
                {filteredPrescriptions.map((prescription) => (
                  <tr key={prescription.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">
                      {prescription.patient?.fullNameEn || `Patient #${prescription.patientId}`}
                    </td>
      ...
      {/* Print Modal */}
      <Dialog open={isPrintModalOpen} onClose={handlePrintModalClose} className="relative z-50">
        <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="mx-auto max-w-4xl w-full bg-white rounded-lg shadow-xl print:shadow-none">
            <div className="p-6 print:p-0">
              ...
              <div>
                {selectedPrescription && <PrintablePrescription prescription={selectedPrescription} />}
              </div>
              <div className="flex justify-end gap-3 mt-6 print:hidden">
                <button onClick={handlePrintModalClose} className="btn btn-secondary">
                  Close
                </button>
...
const PrintablePrescription = ({ prescription }: { prescription: GlassesPrescription }) => {
    ...
    <div className="bg-white p-8 print:p-0">
      <div className="border-2 border-gray-800 p-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">EYEGLASS PRESCRIPTION</h1>
          <p className="text-sm text-gray-600 mt-2">XenonClinic Ophthalmology Department</p>
        </div>
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                {prescription.patient?.dateOfBirth
                  ? format(new Date(prescription.patient.dateOfBirth), 'MMM d, yyyy')
                  : 'N/A'}
              </p>
            </div>
            <div>
          ...
          <table className="w-full border-2 border-gray-800">
            <thead>
              <tr className="bg-gray-100">
                <th className="border border-gray-800 px-4 py-3 text-left font-semibold">Eye</th>
                ...
                <th className="border border-gray-800 px-4 py-3 text-center font-semibold">Axis</th>
                <th className="border border-gray-800 px-4 py-3 text-center font-semibold">
                  Add (Reading)
                </th>
              </tr>
            ...
            <tbody>
              <tr>
                <td className="border border-gray-800 px-4 py-3 font-semibold">Right Eye (OD)</td>
                ...
                <td className="border border-gray-800 px-4 py-3 text-center font-mono text-lg">
                  {prescription.rightAdd ? formatValue(prescription.rightAdd) : '-'}
                </td>
              </tr>
              <tr>
                <td className="border border-gray-800 px-4 py-3 font-semibold">Left Eye (OS)</td>
                <td className="border border-gray-800 px-4 py-3 text-center font-mono text-lg">
                ...
                <td className="border border-gray-800 px-4 py-3 text-center font-mono text-lg">
                  {prescription.leftAdd ? formatValue(prescription.leftAdd) : '-'}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="mb-6">
          ...
          <p className="text-xl font-bold text-gray-900">{prescription.pupillaryDistance} mm</p>
        </div>
        ...
        <div className="mt-6 text-center text-xs text-gray-500">
          <p>This prescription is valid until {format(new Date(prescription.expiryDate), 'MMMM d, yyyy')}</p>
...
const PrescriptionForm = ({ prescription, onSubmit, onCancel, isSubmitting }: PrescriptionFormProps) => {
  const [formData, setFormData] = useState<CreateGlassesPrescriptionRequest>({
    patientId: prescription?.patientId || 0,
    date: prescription?.date ? format(new Date(prescription.date), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="border-t pt-4">
        <h3 className="text-md font-semibold text-gray-900 mb-3">Right Eye (OD)</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Add (Optional)</label>
            <input
              type="number"
              step="0.25"
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  rightAdd: e.target.value ? parseFloat(e.target.value) : undefined,
                })
              }
              className="input w-full"
            />
      ...
      <div className="border-t pt-4">
        <h3 className="text-md font-semibold text-gray-900 mb-3">Left Eye (OS)</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Sphere</label>
            <input
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  leftAdd: e.target.value ? parseFloat(e.target.value) : undefined,
                })
              }
              className="input w-full"
            />
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
          {isSubmitting ? 'Saving...' : prescription ? 'Update Prescription' : 'Create Prescription'}
        </button>
      </div>
    </form>
  );
