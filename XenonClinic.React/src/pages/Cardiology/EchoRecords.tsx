...
export const EchoRecords = () => {
  ...
  const getStatusBadge = (status: EchoStatus) => {
    const config: Record<string, { className: string; label: string }> = {
      [EchoStatus.Scheduled]: {
        className: "bg-blue-100 text-blue-800",
        label: "Scheduled",
      },
      [EchoStatus.InProgress]: {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage echocardiography studies and results
          </p>
        </div>
                ...
                filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {Object.entries(record.valveFindings).map(
                        ([valve, finding]) =>
                          finding &&
                          finding !== ValveFinding.Normal && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      ...
                      {record.status === EchoStatus.Completed &&
  ...
  const handleChange = (
    ...
    if (type === "checkbox") {
      ...
      setFormData((prev) => ({
        ...prev,
        [name]:
          name === "ejectionFraction" || name === "estimatedPAP"
            ? Number(value)
            : value,
      }));
    }
          ...
          <div className="p-6">
            <div className="flex justify-between items-center mb-4">
              <Dialog.Title className="text-lg font-medium text-gray-900">
                {record
                  ? "View Echocardiogram Record"
                  : "New Echocardiogram Record"}
              </Dialog.Title>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {["mitral", "aortic", "tricuspid", "pulmonary"].map(
                    (valve) => (
                      <div key={valve}>
                        ...
                        <select
                          value={
                            formData.valveFindings?.[
                              valve as keyof typeof formData.valveFindings
                            ] || ""
                          }
                          onChange={(e) =>
                            handleValveChange(
                              valve,
                              e.target.value as ValveFinding,
                            )
                          }
                          className="input w-full"
                        >
