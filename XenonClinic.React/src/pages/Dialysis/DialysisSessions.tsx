...
interface DialysisSessionsProps {
  patientId?: number;
}

export const DialysisSessions = ({ patientId }: DialysisSessionsProps = {}) => {
  ...
  const { data: sessions, isLoading } = useQuery<DialysisSession[]>({
    queryKey: ["dialysis-sessions", patientId],
    queryFn: async () => {
      if (patientId) {
        const response = await dialysisApi.getSessionsByPatient(patientId);
        return response.data?.data ?? response.data ?? [];
      }
      const response = await dialysisApi.getAllSessions();
      return response.data?.data ?? response.data ?? [];
    },
  });

  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => dialysisApi.createSession(data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dialysis-sessions"] }),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Record<string, unknown> }) =>
      dialysisApi.updateSession(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dialysis-sessions"] }),
  });
