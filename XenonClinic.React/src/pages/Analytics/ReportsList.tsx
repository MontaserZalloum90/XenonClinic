...
export const ReportsList = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast(
        "success",
        t("message.success.created", "Report generation started successfully"),
      );
    },
    onError: (error: Error) => {
      showToast(
        "error",
        `${t("action.create", "Create")} ${t("message.error.generic", "failed")}: ${error.message}`,
      );
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast(
        "success",
        t("message.success.deleted", "Report deleted successfully"),
      );
    },
    onError: (error: Error) => {
      showToast(
  ...
  const handleDownload = (report: Report) => {
    if (report.fileUrl) {
      ...
      showToast(
        "error",
        t("message.error.noFile", "No file available for download"),
      );
    }
  };
  ...
  const getStatusBadge = (status: number) => {
    const statusConfig = {
      ...
      1: {
        label: t("status.generating", "Generating"),
  ...
  const filteredReports = reports?.filter((report) => {
    ...
    const matchesType =
      selectedType === "all" || report.type.toString() === selectedType;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {t("page.analytics.reports", "Analytics Reports")}
          </h1>
          <p className="text-gray-600 mt-1">
            {t(
              "page.analytics.reportsDescription",
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          {
            label: t("stats.totalReports", "Total Reports"),
      ...
      <div className="card animate-fade-in">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">
              {t("field.reportType", "Report Type")}
            </label>
            <select
              value={selectedType}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mt-4">
              {t("message.loading", "Loading reports...")}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="input w-full"
              placeholder={t(
                "field.reportNamePlaceholder",
                "e.g., Monthly Patient Statistics",
              )}
              required
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  type: Number(e.target.value) as ReportType,
                })
              }
              className="input w-full"
              required
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              ...
              <span className="text-sm text-gray-700">
                {t("field.includeCharts", "Include Charts")}
              </span>
            </label>
            <label className="flex items-center">
              <input
              ...
              />
              <span className="text-sm text-gray-700">
                {t("field.includeDetails", "Include Detailed Data")}
              </span>
            </label>
          </div>
