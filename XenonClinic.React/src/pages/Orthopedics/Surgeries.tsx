...
import type { OrthopedicSurgery, CreateOrthopedicSurgeryRequest } from '../../types/orthopedics';
...
const surgeriesApi = {
  ...
  create: (data: CreateOrthopedicSurgeryRequest) => api.post('/api/OrthopedicsApi/surgeries', data),
  update: (id: number, data: Partial<CreateOrthopedicSurgeryRequest>) => api.put(`/api/OrthopedicsApi/surgeries/${id}`, data),
...
export const Surgeries = () => {
  ...
  const [selectedSurgery, setSelectedSurgery] = useState<OrthopedicSurgery | undefined>(undefined);
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateOrthopedicSurgeryRequest) => surgeriesApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<CreateOrthopedicSurgeryRequest> }) =>
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: CreateOrthopedicSurgeryRequest = {
      ...
      postOpInstructions: formData.get('postOpInstructions') as string || undefined,
  ...
  const handleDelete = (surgery: OrthopedicSurgery) => {
    if (window.confirm(t('message.confirmDelete', `Are you sure you want to delete this surgery record?`))) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('page.surgeries.title', 'Orthopedic Surgeries')}</h1>
          <p className="text-gray-600 mt-1">{t('page.surgeries.description', 'Manage surgical procedures and outcomes')}</p>
        </div>
      ...
      <div className="card animate-fade-in">
        <div className="mb-4">
          <input
            type="text"
            placeholder={t('field.search', 'Search by patient, procedure, surgeon, or indication...')}
            value={searchTerm}
        ...
        {isLoading ? (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredSurgeries.map((surgery) => (
                  <tr key={surgery.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{surgery.patientName || `Patient #${surgery.patientId}`}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{surgery.procedure}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{surgery.indication}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{surgery.surgeon}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {surgery.duration ? `${surgery.duration} min` : '-'}
                    </td>
                    <td className="px-4 py-3 text-sm">
                      {surgery.complications ? (
                        <span className="text-red-600 font-medium">{t('label.yes', 'Yes')}</span>
                      ) : (
                        <span className="text-green-600">{t('label.none', 'None')}</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-sm">
          ...
          <div className="text-center py-8 text-gray-500">
            {searchTerm ? t('message.noResults', 'No surgeries found matching your search.') : t('message.noSurgeries', 'No surgeries recorded yet.')}
          </div>
        )}
      </div>
          ...
          <Dialog.Panel className="mx-auto max-w-4xl w-full bg-white rounded-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <Dialog.Title className="text-lg font-medium text-gray-900 mb-4">
                {selectedSurgery ? t('action.editSurgery', 'Edit Surgery') : t('action.recordSurgery', 'Record Surgery')}
              </Dialog.Title>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      name="surgeryDate"
                      defaultValue={selectedSurgery?.surgeryDate ? format(new Date(selectedSurgery.surgeryDate), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd')}
                      required
                      className="input w-full"
                    />
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {t('field.postOpInstructions', 'Post-operative Instructions')}
                  </label>
                  <textarea
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.patientName || `Patient #${selectedSurgery.patientId}`}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.surgeryDate', 'Surgery Date')}</label>
                      <p className="text-sm text-gray-900 mt-1">{format(new Date(selectedSurgery.surgeryDate), 'MMMM d, yyyy')}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.procedure', 'Procedure')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.procedure}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.indication', 'Indication')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.indication}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.surgeon', 'Surgeon')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.surgeon}</p>
                    </div>
                    {selectedSurgery.assistant && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">{t('field.assistant', 'Assistant')}</label>
                        <p className="text-sm text-gray-900 mt-1">{selectedSurgery.assistant}</p>
                      </div>
                    )}
                    {selectedSurgery.anesthesia && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">{t('field.anesthesia', 'Anesthesia')}</label>
                        <p className="text-sm text-gray-900 mt-1">{selectedSurgery.anesthesia}</p>
                      </div>
                    )}
                    {selectedSurgery.duration && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">{t('field.duration', 'Duration')}</label>
                        <p className="text-sm text-gray-900 mt-1">{selectedSurgery.duration} minutes</p>
                      </div>
                    )}
                  </div>
                  {selectedSurgery.implants && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.implants', 'Implants/Hardware')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.implants}</p>
                    </div>
                  )}
                  {selectedSurgery.complications && (
                    <div className="border-t pt-4">
                      ...
                      <p className="text-sm text-red-600 mt-1">{selectedSurgery.complications}</p>
                    </div>
                  )}
                  {selectedSurgery.postOpInstructions && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.postOpInstructions', 'Post-operative Instructions')}</label>
                      <p className="text-sm text-gray-900 mt-1 whitespace-pre-wrap">{selectedSurgery.postOpInstructions}</p>
                    </div>
                  )}
                  {selectedSurgery.outcome && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.outcome', 'Outcome')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.outcome}</p>
                    </div>
                  )}
                  {selectedSurgery.notes && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">{t('field.notes', 'Notes')}</label>
                      <p className="text-sm text-gray-900 mt-1">{selectedSurgery.notes}</p>
                    </div>
                  )}
