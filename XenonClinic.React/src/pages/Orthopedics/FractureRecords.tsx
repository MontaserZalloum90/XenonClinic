...
export const FractureRecords = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateFractureRecordRequest) =>
      fracturesApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: number;
      data: Partial<CreateFractureRecordRequest>;
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: CreateFractureRecordRequest = {
      ...
      expectedHealingTime: formData.get("expectedHealingTime")
        ? parseInt(formData.get("expectedHealingTime") as string)
        : undefined,
    ...
    if (
      window.confirm(
        t(
          ...
          `Are you sure you want to delete this fracture record?`,
        ),
      )
    ) {
  ...
  const filteredFractures = fractures?.filter((fracture) => {
    ...
    const matchesStatus =
      statusFilter === "" || fracture.status === statusFilter;
      ...
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {t("page.fractureRecords.title", "Fracture Records")}
          </h1>
          <p className="text-gray-600 mt-1">
            {t(
              "page.fractureRecords.description",
              "Track fractures and healing progress",
            )}
          </p>
        </div>
        ...
        <div className="mb-4 flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <input
              ...
              placeholder={t(
                "field.search",
                "Search by patient, bone, type, or location...",
              )}
              value={searchTerm}
          ...
          <div>
            ...
            >
              ...
              <option value="3">
                {t("status.complicated", "Complicated")}
              </option>
            </select>
          </div>
        </div>
        ...
        {isLoading ? (
                ...
                {filteredFractures.map((fracture) => (
                  <tr key={fracture.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      {fracture.patientName || `Patient #${fracture.patientId}`}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {fracture.boneAffected}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(fracture.status)}`}
                      >
                        {getStatusLabel(fracture.status)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                      {fracture.followUpDate
                        ? format(new Date(fracture.followUpDate), "MMM d, yyyy")
                        : "-"}
                    </td>
                    <td className="px-4 py-3 text-sm">
          ...
          <div className="text-center py-8 text-gray-500">
            {searchTerm || statusFilter !== ""
              ? t(
                  "message.noResults",
                  "No fracture records found matching your filters.",
                )
          ...
          <Dialog.Panel className="mx-auto max-w-4xl w-full bg-white rounded-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <Dialog.Title className="text-lg font-medium text-gray-900 mb-4">
                {selectedFracture
                  ? t("action.editFracture", "Edit Fracture")
                  : t("action.recordFracture", "Record Fracture")}
              </Dialog.Title>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="fractureDate"
                      defaultValue={
                        selectedFracture?.fractureDate
                          ? format(
                              new Date(selectedFracture.fractureDate),
                              "yyyy-MM-dd",
                            )
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {t(
                        "field.expectedHealingTime",
                        "Expected Healing Time (weeks)",
                      )}
                    </label>
                    <input
                      type="number"
                    ...
                    <input
                      ...
                      defaultValue={
                        selectedFracture?.followUpDate
                          ? format(
                              new Date(selectedFracture.followUpDate),
                              "yyyy-MM-dd",
                            )
                          : ""
                      }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900 mt-1">
                        {selectedFracture.patientName ||
                          `Patient #${selectedFracture.patientId}`}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                    ...
                    {selectedFracture.expectedHealingTime && (
                      <div>
                        ...
                        <p className="text-sm text-gray-900 mt-1">
                          {selectedFracture.expectedHealingTime} weeks
                        </p>
                      </div>
                    )}
                    {selectedFracture.followUpDate && (
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-block px-2 py-1 rounded-full text-xs font-medium mt-1 ${getStatusColor(selectedFracture.status)}`}
                      >
