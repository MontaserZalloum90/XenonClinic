...
export const AdminDashboard = () => {
  ...
  const handleResetDemo = async () => {
    if (
      !confirm(
        "Are you sure you want to reset the demo data? This will clear all current data and reload fresh demo data.",
      )
    ) {
      return;
    }
  ...
  const handleClearDemo = async () => {
    if (
      !confirm(
        "Are you sure you want to clear ALL demo data? This cannot be undone.",
      )
    ) {
      return;
    }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            System settings and demo data management
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Customize labels and terminology
                </p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <StatCard
                    icon={UserGroupIcon}
                    label="Patients"
                    value={demoStatus.stats.patients}
                    color="blue"
                  />
                  <StatCard
                    icon={CalendarIcon}
                    label="Appointments"
                    value={demoStatus.stats.appointments}
                    color="green"
                  />
                  <StatCard
                    icon={ClipboardDocumentListIcon}
                    label="Encounters"
                    value={demoStatus.stats.encounters}
                    color="purple"
                  />
                  <StatCard
                    icon={BuildingStorefrontIcon}
                    label="Inventory"
                    value={demoStatus.stats.inventory}
                    color="yellow"
                  />
                </div>
                {demoStatus.loadedAt && (
              ...
              {clinicTypes.map((clinic) => (
                ...
                >
                  <div className="font-medium text-gray-900">
                    {clinic.name.replace("XenonClinic ", "")}
                  </div>
                  <div className="text-xs text-gray-500 mt-1 line-clamp-2">
                    {clinic.description}
                  </div>
                  {demoStatus?.clinicType === clinic.type &&
                    demoStatus.isLoaded && (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Paste the exported JSON data below. This will replace all
                current demo data.
              </p>
              <textarea
                value={importData}
