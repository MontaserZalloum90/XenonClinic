...
import type { TenantContext, FeatureMap, NavItem } from "../types/tenant";
...
const mockClinicContext: TenantContext = {
  ...
  listLayouts: {
    patient: {
      entityName: "patient",
      columns: [{ field: "fullName", sortable: true }, { field: "email" }],
      actions: {
...
describe("Conditional Rules", () => {
  ...
  it("evaluates empty/notEmpty conditions", () => {
    expect(
      evaluateCondition({ field: "email", operator: "empty" }, { email: "" }),
    ).toBe(true);
    expect(
      evaluateCondition({ field: "email", operator: "empty" }, { email: null }),
    ).toBe(true);
    expect(
      evaluateCondition(
