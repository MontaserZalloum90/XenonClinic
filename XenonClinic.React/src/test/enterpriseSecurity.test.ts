...
describe("File Upload Security", () => {
  ...
  describe("createSecureFilePath", () => {
    it("creates safe path without traversal", () => {
      const path = createSecureFilePath(
        "/uploads",
        "../../../etc/passwd",
        "users",
      );
...
describe("Password Policy", () => {
  ...
  describe("HIPAA_PASSWORD_POLICY", () => {
    it("has stricter requirements than default", () => {
      expect(HIPAA_PASSWORD_POLICY.minLength).toBeGreaterThan(
...
describe("Account Lockout", () => {
  ...
  it("returns attempts remaining message", () => {
    ...
    expect(result.attemptsRemaining).toBe(
      DEFAULT_LOCKOUT_POLICY.maxFailedAttempts - 1,
    );
...
describe("CSRF Protection", () => {
  ...
  describe("validateOrigin", () => {
    it("validates exact origin match", () => {
      ...
      expect(validateOrigin("https://evil.com", ["https://example.com"])).toBe(
        false,
      );
    });
    ...
    it("validates wildcard subdomains", () => {
      ...
      expect(
        validateOrigin("https://sub.app.example.com", ["*.example.com"]),
      ).toBe(true);
    });
