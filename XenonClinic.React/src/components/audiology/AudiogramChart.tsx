...
const RightEarDot = (props: DotProps) => {
  ...
  if (payload?.rightAirNoResponse) {
    ...
    return (
      <g>
        ...
        <path
          d={`M${cx},${cy - 3} L${cx},${cy + 3} M${cx - 2},${cy + 1} L${cx},${cy + 3} L${cx + 2},${cy + 1}`}
          stroke="white"
          strokeWidth={1.5}
          fill="none"
        />
...
const LeftEarDot = (props: DotProps) => {
  const { cx, cy, payload } = props;
...
interface SimpleDotProps {
  cx?: number;
  cy?: number;
}

const RightBoneDot = (props: SimpleDotProps) => {
  const { cx, cy } = props;
...
const LeftBoneDot = (props: SimpleDotProps) => {
  const { cx, cy } = props;
    ...
    <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
      ...
      {payload.map((entry: TooltipPayloadEntry, index: number) => (
        <p key={index} style={{ color: entry.color }} className="text-sm">
          {entry.name}:{" "}
          {entry.value !== null ? `${entry.value} dB` : "No Response"}
        </p>
      ))}
    </div>
  );
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
        <div className="bg-red-50 rounded-lg p-3">
          ...
          <p className="text-red-700">
            PTA:{" "}
            {audiogram.rightPTA !== undefined
              ? `${audiogram.rightPTA} dB`
              : "N/A"}
          </p>
          <p className="text-red-700">
        ...
        <div className="bg-blue-50 rounded-lg p-3">
          ...
          <p className="text-blue-700">
            PTA:{" "}
            {audiogram.leftPTA !== undefined
              ? `${audiogram.leftPTA} dB`
              : "N/A"}
          </p>
          <p className="text-blue-700">
  ...
  <div className="flex flex-wrap gap-2 text-xs">
    {GRADE_BOUNDARIES.map((grade, index) => (
      <div key={grade.label} className="flex items-center">
        ...
        <span>
          {grade.label} ({index === 0 ? "≤" : ">"}
          {index > 0 ? GRADE_BOUNDARIES[index - 1].threshold : -10}-
          {grade.threshold} dB)
        </span>
      </div>
    ))}
...
export const calculatePTA = (
  dataPoints: AudiogramDataPoint[],
): number | undefined => {
  const frequencies: AudiogramFrequency[] = [500, 1000, 2000, 4000];
  const relevantPoints = dataPoints.filter(
    (p) => frequencies.includes(p.frequency) && !p.noResponse,
  );
...
export const getHearingLossGrade = (
  pta: number | undefined,
): HearingLossGrade | undefined => {
