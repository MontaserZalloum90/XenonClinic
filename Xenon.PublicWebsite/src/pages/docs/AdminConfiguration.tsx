...
import {
  ...
  HardDrive, Cloud, Zap, Monitor, Headphones, Languages, Stethoscope, Archive,
  Activity, Download, Upload, Eye, History, Clipboard, Calculator, Thermometer,
} from 'lucide-react';
...
const configCategories: ConfigCategory[] = [
  ...
  },
  {
    ...
    icon: Languages,
    description: 'Multi-language support, regional settings, and cultural preferences',
    settings: [
      { name: 'Available Languages', type: 'multiselect', description: 'Languages users can choose from', defaultValue: 'English', example: 'English, Arabic, French, Urdu, Hindi, Chinese' },
      { name: 'Default Language', type: 'select', description: 'Default language for new users', defaultValue: 'English (US)', required: true },
      { name: 'RTL Support', type: 'toggle', description: 'Enable right-to-left layout for Arabic/Hebrew', defaultValue: 'Auto' },
      { name: 'Date Format', type: 'select', description: 'Regional date display format', defaultValue: 'DD/MM/YYYY', example: 'DD/MM/YYYY, MM/DD/YYYY, YYYY-MM-DD' },
      { name: 'Time Format', type: 'select', description: '12-hour or 24-hour clock', defaultValue: '12-hour', example: '12-hour (AM/PM), 24-hour' },
      { name: 'Number Format', type: 'select', description: 'Decimal and thousands separators', defaultValue: '1,234.56', example: '1,234.56 (US), 1.234,56 (EU)' },
      { name: 'Currency Symbol Position', type: 'select', description: 'Currency symbol before or after amount', defaultValue: 'Before', example: 'Before ($100), After (100$)' },
      { name: 'First Day of Week', type: 'select', description: 'Calendar week start day', defaultValue: 'Sunday', example: 'Sunday, Monday, Saturday' },
      { name: 'Default Country Code', type: 'select', description: 'Default phone country code', defaultValue: '+971 (UAE)', required: true },
      { name: 'Address Format', type: 'select', description: 'Regional address field order', defaultValue: 'UAE Format' },
      { name: 'Emirates ID Validation', type: 'toggle', description: 'Enable UAE Emirates ID format validation', defaultValue: 'On' },
      { name: 'Translation Mode', type: 'select', description: 'How missing translations are handled', defaultValue: 'Fall back to English' },
      { name: 'Patient Portal Language', type: 'select', description: 'Default language for patient-facing pages', defaultValue: 'Same as system' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    icon: Stethoscope,
    description: 'Clinical workflow, medical coding, vital signs, and healthcare-specific settings',
    settings: [
      { name: 'ICD Version', type: 'select', description: 'Diagnosis coding system version', defaultValue: 'ICD-10-CM', example: 'ICD-10-CM, ICD-10-AM, ICD-11' },
      { name: 'CPT Coding', type: 'toggle', description: 'Enable CPT procedure coding', defaultValue: 'On' },
      { name: 'Drug Database', type: 'select', description: 'Medication database source', defaultValue: 'WHO Essential Medicines', example: 'WHO, FDA NDC, Local formulary' },
      { name: 'Drug Interaction Alerts', type: 'select', description: 'Severity level for drug interaction warnings', defaultValue: 'Moderate and above', example: 'All, Moderate+, Severe only' },
      { name: 'Allergy Alert Level', type: 'select', description: 'When to show allergy warnings', defaultValue: 'All allergies', example: 'All, Severe only, Drug allergies only' },
      { name: 'Vital Signs Units (Weight)', type: 'select', description: 'Unit for patient weight', defaultValue: 'kg', example: 'kg, lbs' },
      { name: 'Vital Signs Units (Height)', type: 'select', description: 'Unit for patient height', defaultValue: 'cm', example: 'cm, ft/in' },
      { name: 'Vital Signs Units (Temperature)', type: 'select', description: 'Temperature measurement unit', defaultValue: 'Celsius', example: 'Celsius, Fahrenheit' },
      { name: 'Blood Pressure Format', type: 'select', description: 'BP display format', defaultValue: 'Systolic/Diastolic' },
      { name: 'BMI Calculation', type: 'toggle', description: 'Auto-calculate BMI from height/weight', defaultValue: 'On' },
      { name: 'Clinical Note Templates', type: 'toggle', description: 'Enable SOAP and specialty templates', defaultValue: 'On' },
      { name: 'Require Diagnosis for Billing', type: 'toggle', description: 'Mandate diagnosis before invoice', defaultValue: 'On' },
      { name: 'E-Prescribing', type: 'toggle', description: 'Enable electronic prescriptions', defaultValue: 'On' },
      { name: 'Prescription Validity Days', type: 'number', description: 'Default prescription expiry', defaultValue: '30' },
      { name: 'Controlled Substance Rules', type: 'select', description: 'Controlled medication handling', defaultValue: 'UAE MOH regulations' },
      { name: 'Lab Reference Ranges', type: 'select', description: 'Source for normal value ranges', defaultValue: 'WHO/CDC standards' },
      { name: 'Critical Value Alerts', type: 'toggle', description: 'Alert for critical lab results', defaultValue: 'On', required: true },
      { name: 'Critical Value Notification', type: 'multiselect', description: 'Who receives critical alerts', example: 'Ordering physician, Nurse, Admin' },
      { name: 'Visit Auto-lock Hours', type: 'number', description: 'Hours after which unsigned visits lock', defaultValue: '72' },
      { name: 'Amendment Window Days', type: 'number', description: 'Days to amend signed records', defaultValue: '7' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    icon: Archive,
    description: 'Data retention, archiving, export, and storage management settings',
    settings: [
      { name: 'Data Retention Period', type: 'select', description: 'How long to keep patient records', defaultValue: '10 years', example: '5, 7, 10, 15 years, Indefinite' },
      { name: 'Inactive Patient Archive', type: 'number', description: 'Years until patients marked inactive', defaultValue: '3' },
      { name: 'Auto-archive Documents', type: 'toggle', description: 'Move old documents to cold storage', defaultValue: 'On' },
      { name: 'Archive After Days', type: 'number', description: 'Days before documents auto-archive', defaultValue: '365' },
      { name: 'Audit Log Retention', type: 'select', description: 'How long to keep audit logs', defaultValue: '7 years', example: '1, 3, 5, 7 years' },
      { name: 'Deleted Record Retention', type: 'number', description: 'Days to keep soft-deleted records', defaultValue: '90' },
      { name: 'Export Format', type: 'select', description: 'Default data export format', defaultValue: 'Excel (XLSX)', example: 'CSV, XLSX, PDF, JSON' },
      { name: 'Export Encryption', type: 'toggle', description: 'Password-protect exported files', defaultValue: 'On' },
      { name: 'Bulk Export Limit', type: 'number', description: 'Maximum records per export', defaultValue: '10000' },
      { name: 'FHIR Export', type: 'toggle', description: 'Enable FHIR format for interoperability', defaultValue: 'On' },
      { name: 'Storage Location', type: 'select', description: 'Primary data storage region', defaultValue: 'UAE (Dubai)', example: 'UAE, US, EU' },
      { name: 'Storage Encryption', type: 'readonly', description: 'Data at rest encryption', defaultValue: 'AES-256 (Always On)' },
      { name: 'Media Storage Limit', type: 'readonly', description: 'Total storage for images/documents' },
      { name: 'Database Size Alert', type: 'number', description: 'Alert when DB reaches % capacity', defaultValue: '80' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    icon: HardDrive,
    description: 'Data backup schedules, disaster recovery, and business continuity settings',
    settings: [
      { name: 'Auto Backup', type: 'toggle', description: 'Enable automated backups', defaultValue: 'On', required: true },
      { name: 'Backup Frequency', type: 'select', description: 'How often to create backups', defaultValue: 'Daily', example: 'Hourly, Daily, Weekly' },
      { name: 'Backup Time', type: 'time', description: 'Scheduled backup time (off-peak recommended)', defaultValue: '02:00 AM' },
      { name: 'Backup Retention', type: 'select', description: 'How long to keep backup copies', defaultValue: '30 days', example: '7, 14, 30, 90 days' },
      { name: 'Backup Type', type: 'select', description: 'Full or incremental backups', defaultValue: 'Daily incremental, Weekly full' },
      { name: 'Backup Location', type: 'multiselect', description: 'Where backups are stored', defaultValue: 'Cloud (Primary), Off-site (Secondary)' },
      { name: 'Backup Encryption', type: 'readonly', description: 'Backup encryption status', defaultValue: 'AES-256 (Always On)' },
      { name: 'Point-in-Time Recovery', type: 'toggle', description: 'Enable granular recovery options', defaultValue: 'On' },
      { name: 'Recovery Point Objective', type: 'select', description: 'Maximum acceptable data loss', defaultValue: '1 hour', example: '15 min, 1 hour, 4 hours, 24 hours' },
      { name: 'Recovery Time Objective', type: 'select', description: 'Maximum acceptable downtime', defaultValue: '4 hours', example: '1 hour, 4 hours, 8 hours, 24 hours' },
      { name: 'Disaster Recovery Site', type: 'toggle', description: 'Enable secondary DR environment', defaultValue: 'On' },
      { name: 'DR Failover Mode', type: 'select', description: 'Automatic or manual failover', defaultValue: 'Manual with notification' },
      { name: 'Backup Verification', type: 'toggle', description: 'Test backup integrity automatically', defaultValue: 'On' },
      { name: 'Backup Notification', type: 'multiselect', description: 'Who receives backup status alerts', example: 'System Admin, IT Team' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    settings: [
      { name: 'Audit Logging', type: 'readonly', description: 'Comprehensive audit trail', defaultValue: 'Always On (Cannot disable)' },
      { name: 'Audit Detail Level', type: 'select', description: 'Level of detail in audit logs', defaultValue: 'Detailed', example: 'Basic, Standard, Detailed' },
      { name: 'PHI Access Logging', type: 'toggle', description: 'Log all patient record access', defaultValue: 'On', required: true },
      { name: 'Failed Login Logging', type: 'toggle', description: 'Log failed authentication attempts', defaultValue: 'On' },
      { name: 'Export Activity Logging', type: 'toggle', description: 'Log all data exports', defaultValue: 'On' },
      { name: 'Audit Log Access', type: 'select', description: 'Who can view audit logs', defaultValue: 'System Admin only' },
      { name: 'Audit Log Export', type: 'select', description: 'Who can export audit logs', defaultValue: 'System Admin with MFA' },
      { name: 'Compliance Framework', type: 'multiselect', description: 'Applicable compliance standards', example: 'HIPAA, DOH UAE, ISO 27001, GDPR' },
      { name: 'HIPAA Mode', type: 'toggle', description: 'Enable HIPAA-specific controls', defaultValue: 'On' },
      { name: 'Break the Glass', type: 'toggle', description: 'Emergency access to restricted records', defaultValue: 'On' },
      { name: 'Break Glass Justification', type: 'toggle', description: 'Require reason for emergency access', defaultValue: 'On' },
      { name: 'Consent Tracking', type: 'toggle', description: 'Track patient consent records', defaultValue: 'On' },
      { name: 'Data Processing Agreement', type: 'toggle', description: 'Require DPA for data sharing', defaultValue: 'On' },
      { name: 'Compliance Report Schedule', type: 'select', description: 'Auto-generate compliance reports', defaultValue: 'Monthly', example: 'Weekly, Monthly, Quarterly' },
      { name: 'Compliance Officer Email', type: 'email', description: 'Email for compliance notifications' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    settings: [
      { name: 'Default Currency', type: 'select', description: 'Primary currency for transactions', defaultValue: 'AED', required: true },
      { name: 'Secondary Currencies', type: 'multiselect', description: 'Additional accepted currencies', example: 'USD, EUR, GBP' },
      { name: 'Exchange Rate Update', type: 'select', description: 'How exchange rates are updated', defaultValue: 'Daily automatic' },
      { name: 'Tax Type', type: 'select', description: 'Tax system type', defaultValue: 'VAT', example: 'VAT, GST, Sales Tax, None' },
      { name: 'Tax Rate', type: 'number', description: 'Default tax percentage', defaultValue: '5', example: '5% UAE VAT' },
      { name: 'Tax Registration Number', type: 'text', description: 'TRN for tax invoices', required: true },
      { name: 'Price Rounding', type: 'select', description: 'How prices are rounded', defaultValue: 'Nearest 0.01' },
      { name: 'Invoice Numbering', type: 'select', description: 'Invoice number format', defaultValue: 'INV-YYYY-NNNNNN' },
      { name: 'Invoice Due Days', type: 'number', description: 'Default payment terms', defaultValue: '30' },
      { name: 'Late Payment Fee', type: 'toggle', description: 'Apply late payment charges', defaultValue: 'Off' },
      { name: 'Payment Methods', type: 'multiselect', description: 'Accepted payment types', defaultValue: 'Cash, Card, Insurance', example: 'Cash, Card, Bank Transfer, Insurance' },
      { name: 'Advance Payment Required', type: 'toggle', description: 'Require deposit for appointments', defaultValue: 'Off' },
      { name: 'Insurance Auto-verification', type: 'toggle', description: 'Auto-check insurance eligibility', defaultValue: 'On' },
      { name: 'Claim Auto-submission', type: 'toggle', description: 'Auto-submit insurance claims', defaultValue: 'Off' },
      { name: 'Revenue Recognition', type: 'select', description: 'When revenue is recognized', defaultValue: 'On service delivery' },
      { name: 'Fiscal Year Start', type: 'select', description: 'Financial year start month', defaultValue: 'January' },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    icon: Zap,
    description: 'Automated workflows, task assignments, and process automation',
    settings: [
      { name: 'Workflow Engine', type: 'toggle', description: 'Enable visual workflow builder', defaultValue: 'On' },
      { name: 'Auto-assign Tasks', type: 'toggle', description: 'Automatically assign tasks based on rules', defaultValue: 'On' },
      { name: 'Escalation Rules', type: 'toggle', description: 'Enable automatic task escalation', defaultValue: 'On' },
      { name: 'Escalation Timeout', type: 'number', description: 'Hours before task escalates', defaultValue: '24' },
      { name: 'Appointment Auto-confirm', type: 'toggle', description: 'Auto-confirm appointments', defaultValue: 'Off' },
      { name: 'Reminder Automation', type: 'toggle', description: 'Send automated reminders', defaultValue: 'On' },
      { name: 'No-show Follow-up', type: 'toggle', description: 'Auto-send no-show notifications', defaultValue: 'On' },
      { name: 'Follow-up Reminders', type: 'toggle', description: 'Remind patients about follow-ups', defaultValue: 'On' },
      { name: 'Lab Result Auto-release', type: 'toggle', description: 'Auto-release normal results', defaultValue: 'Off' },
      { name: 'Prescription Refill Alerts', type: 'toggle', description: 'Notify for medication refills', defaultValue: 'On' },
      { name: 'Pending Task Dashboard', type: 'toggle', description: 'Show pending tasks on login', defaultValue: 'On' },
      { name: 'Workflow Notifications', type: 'multiselect', description: 'How workflow events notify users', example: 'In-app, Email, SMS, Push' },
      { name: 'Daily Digest', type: 'toggle', description: 'Send daily task summary email', defaultValue: 'On' },
      { name: 'Workflow Audit Trail', type: 'toggle', description: 'Log all workflow actions', defaultValue: 'On' },
    ],
    setupSteps: [
